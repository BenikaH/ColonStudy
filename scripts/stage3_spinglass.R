...
setwd("/Users/BC/Desktop/GeNets_Analysis_Stage3/stage3/")
genes <- read.csv("stage3_connected.csv", header=FALSE)
g=as.matrix(genes)
...
sg9 <- spinglass.community(y, weights = NULL, update.rule="config",start.temp=1, stop.temp=.30, gamma =2)
...
cc <-get.edgelist(comGraphBig1)s
ccs
write.csv(cc, "results_default/stage3_comm1.csv")
