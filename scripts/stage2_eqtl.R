...
setwd("/Users/BC/Documents/clinical/eqtl/results/test/stage2/")

miR <- as.data.frame(read.table("stage2_mirna_matrix.txt", sep="\t", header=TRUE, row.names=1))
rna <- as.data.frame(read.table("stage2_rna_matrix.txt", sep="\t", header=TRUE,row.names=1))
...
for(i in 1:nrow(x1)){
  ...
  if(sum(index!=0) > 74){ x2 <- rbind(x2,x1[i,]);}
}
...
for(i in 1:nrow(y1)){
  ...
  if(sum(index!=0) > 74){ y2 <- rbind(y2,y1[i,]);}
}

z.mirna <- scale(x2, center = TRUE, scale = TRUE)
z.rna <- scale(y2, center = TRUE, scale = TRUE)

write.table(x2, file="stage2_mirna_80.txt",row.names=T, sep="\t")
write.table(y2, file="stage2_rna_80.txt",row.names=T, sep="\t")

miR <- as.data.frame(read.table("stage2_mirna_80.txt", sep="\t", header=TRUE, row.names=1))
rna <- as.data.frame(read.table("stage2_rna_80.txt", sep="\t", header=TRUE,row.names=1))

z.mirna <- scale(miR, center = TRUE, scale = TRUE)
z.rna <- scale(rna, center = TRUE, scale = TRUE)

write.table(z.mirna, file="stage2_mirna_sd.txt",row.names=T, sep="\t")
write.table(z.rna, file="stage2_rna_sd.txt",row.names=T, sep="\t")


miR <- as.data.frame(read.table("stage2_mirna_sd.txt", sep="\t", header=TRUE, row.names=1))
rna <- as.data.frame(read.table("stage2_rna_sd.txt", sep="\t", header=TRUE,row.names=1))
...
#inverse_quantile_norm('stage2_mirna_sd.txt')

library(preprocessCore)

#Quantile normalization of miRNA data
m <- as.matrix(read.table("stage2_mirna_sd.txt", header=TRUE, row.names = 1))
#dimnames(m) <- NULL
m <- as.matrix(read.table("stage2_rna_sd.txt", header=TRUE, row.names = 1))
...
n<- as.matrix(n)
#Write to file
write.table(n, file = "quantile_stage2_rna_sd.txt", sep = "\t")




miR <- as.data.frame(read.table("quantile_stage2_mirna_sd.txt", sep="\t", header=TRUE, row.names=1))
rna <- as.data.frame(read.table("quantile_stage2_rna_sd.txt", sep="\t", header=TRUE,row.names=1))

old_colnames <- colnames(miR)

mirna_pc <- princomp(~., data=miR, center=F, scale=F, na.action=na.omit)
pc1_5 <- as.data.frame(mirna_pc$loadings[,1:6])
#rownames(pc1_5) <- oldsnpcolnames
colnames(pc1_5) <- c("PC1","PC2", "PC3", "PC4", "PC5", "PC6")
pc1_5 <- t(pc1_5)
pc1_5<- cbind(rownames(pc1_5), pc1_5)
colnames(pc1_5)[1] = "ID"
write.table(pc1_5,paste0('stage2_mirna_pcs2.txt'),col.names=T,row.names=F,quote=F,sep="\t")

pdf(paste0('stage2_mirna_screeplot2.pdf'))
screeplot(mirna_pc, type='lines')
dev.off()
p1p2 <- cbind(mirna_pc$loadings[,1],mirna_pc$loadings[,2],mirna_pc$loadings[,3],mirna_pc$loadings[,4],mirna_pc$loadings[,5],mirna_pc$loadings[,6])
#
...
colnames(pc1pc2) <- c("PC1","PC2", "PC3", "PC4", "PC5", "PC6", 'Race')
...
ggsave(paste0('stage2_race_pc1_pc2.pdf'),plot=pcaplot)
...
colnames(pc1pc2.gender) <- c("PC1","PC2", "PC3", "PC4", "PC5", "PC6",'Gender')
#
...
ggsave(paste0('stage2_mirna_pc1_pc2-gender2.pdf'),plot=pcaplot)

#####PC Analysis gene expression#####
gene_pc <- princomp(~., data=rna, center=F, scale=F, na.action=na.omit)
gene_pc1_5 <- as.data.frame(gene_pc$loadings[,1:6])
rownames(gene_pc1_5) <- old_colnames
colnames(gene_pc1_5) <- c("PC1","PC2", "PC3", "PC4", "PC5", "PC6")
gene_pc1_5 <- t(gene_pc1_5)
gene_pc1_5<- cbind(rownames(gene_pc1_5), gene_pc1_5)
colnames(gene_pc1_5)[1] = "ID"
write.table(gene_pc1_5,paste0('gene_pcs2.txt'),col.names=T,row.names=F,quote=F,sep="\t")

pdf(paste0('stage2_gene_expression-screeplot2.pdf'))
screeplot(gene_pc, type='lines')
dev.off()
gene_p1p2 <- cbind(gene_pc$loadings[,1],gene_pc$loadings[,2],gene_pc$loadings[,3],gene_pc$loadings[,4],gene_pc$loadings[,5],gene_pc$loadings[,6])
#
...
colnames(gene_pc1pc2) <- c("PC1","PC2", "PC3", "PC4", "PC5", "PC6", 'Race')
...
ggsave(paste0('stage2_gene_pc1_pc2-2.pdf'),plot=gene_pcaplot)

gene_gender <- read.table(paste0('gender.txt'),header=T,sep="\t", row.names=1,na.strings='NA',check.names=F,stringsAsFactors=T)
...
colnames(pc1pc2) <- c("PC1","PC2", "PC3", "PC4", "PC5", "PC6",'Gender')
#
...
ggsave(paste0('stage2_gene_pc1_pc2-gender2.pdf'),plot=gene_pcaplot)

p1p2 <- cbind(gene_pc$loadings[,1],gene_pc$loadings[,2])
pc1pc2<-cbind.data.frame(p1p2,race$race[match(rownames(p1p2),rownames(race))])

pdf('stage2_gene-iqnorm-pc1pc2-2.pdf')
...
#cov <- as.matrix(read.table("stage2_covariates.txt",header=T,row.names=1, sep="\t"))

snps = SlicedData$new()$LoadFile("quantile_stage2_mirna_sd.txt");
#covariates_file = cov
cvrt = SlicedData$new()$LoadFile('stage2_cov.txt')
gene = SlicedData$new()$LoadFile("quantile_stage2_rna_sd.txt");
...
me = Matrix_eQTL_main(
  ...
  gene = gene,
  cvrt = cvrt,
  output_file_name <- "qnorm_stage_2_trans_only_nodups.txt",
  output_file_name.cis = "qnorm_stage_2_cis_meqtl_nodups_res.txt",
